...
import { AlertTriangle, CalendarClock, CircleUser, Inbox, PackageOpen, ShieldAlert } from 'lucide-react';
   ...
   },
   {
      ...
      predicate: (i) =>
         ...
         (i.labels.some((l) => /secur|token|secret|auth|access/i.test(l.name)) ||
            /secur|token|secret|rotate|credential|password/i.test(`${i.title} ${i.description ?? ''}`)),
   },
];
...
export function SavedView({ viewKey }: { viewKey: string }) {
   ...
   const displayed = useMemo(() => {
      if (!view) return [];
      return issues.filter((i) => view.predicate(i, { currentUserId }));
   }, [issues, view, currentUserId]);
   ...
   if (displayed.length === 0) {
      ...
      return (
         <div className="flex h-full w-full flex-col items-center justify-center gap-2 p-10 text-center">
            ...
            <p className="max-w-xs text-xs text-muted-foreground">{view.description} — all clear right now.</p>
         </div>
      );
   }
