...
function breakdown(mine: Issue[]) {
   return {
      ...
      todo: mine.filter((i) => ['unstarted', 'backlog', 'triage'].includes(i.status.category)).length,
...
export function ProjectsView() {
   ...
   const rows = useMemo(
      () =>
         projects.map((p) => {
            const mine = issues.filter((i) => i.project?.id === p.id).sort((a, b) => b.rank.localeCompare(a.rank));
            const b = breakdown(mine);
            const pct = mine.length ? Math.round((b.done / mine.length) * 100) : 0;
            const owners = Array.from(
               new Map(mine.filter((i) => i.assignee).map((i) => [i.assignee!.id, i.assignee!])).values()
            );
      ...
      <div className="mx-auto w-full max-w-5xl space-y-5 p-4 sm:p-6">
         ...
         <div className="grid gap-4 md:grid-cols-2">
            {rows.map(({ p, mine, inProgress, todo, done, pct, owners }) => {
               ...
               return (
                  <div key={p.id} className="rounded-xl border bg-container p-4 transition-colors hover:border-[#5e6ad2]/50">
                     ...
                     <div className="mt-3">
                        <div className="mb-1 flex items-center justify-between text-[11px] text-muted-foreground">
                           <span>{done}/{mine.length} done</span>
                           <span>{pct}%</span>
                        </div>
                     ...
                     <div className="mt-3 flex flex-wrap gap-1.5 text-[11px]">
                        <span className="rounded-md bg-amber-500/10 px-2 py-0.5 text-amber-500">{inProgress} in progress</span>
                        <span className="rounded-md bg-muted/50 px-2 py-0.5 text-muted-foreground">{todo} to do</span>
                        <span className="rounded-md bg-emerald-500/10 px-2 py-0.5 text-emerald-500">{done} done</span>
                     </div>
                        ...
                        <div className="mt-3 space-y-1 border-t pt-3">
                           {mine.slice(0, 3).map((i) => (
                              <div key={i.id} className="flex items-center gap-2 text-xs">
                                 <CircleDot className="size-3 shrink-0" style={{ color: i.status.color }} />
                                 <span className="truncate text-muted-foreground">{i.identifier}</span>
                                 <span className="truncate">{i.title}</span>
                              </div>
                           ))}
                           {mine.length > 3 && (
                              <div className="pl-5 text-[11px] text-muted-foreground">+{mine.length - 3} more</div>
                           )}
                        </div>
                     )}
                     ...
                     <div className="mt-3 flex items-center justify-between">
                        {p.lead?.id !== 'unassigned' ? (
                           <span className="text-[11px] text-muted-foreground">Lead: {p.lead.name}</span>
                        ) : <span />}
