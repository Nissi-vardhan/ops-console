...
function rawDigest(day: string, tasks: DayTask[]): string {
   ...
   for (const t of tasks) {
      ...
      for (const n of t.notes)
         lines.push(`  • ${n.time}${n.session ? ` [${n.session}]` : ''} ${n.text}`);
      lines.push('');
   }
   return lines.join('\n');
}
...
function extractJson(txt: string): DailyData {
   try {
      ...
      if (m) {
         try {
            return normalizeData(JSON.parse(m[0]));
         } catch {
            /* give up */
         }
      }
...
// Prefer Claude (Anthropic) when configured; fall back to OpenAI. The compile is
// only as good as the day's task notes — it stays strictly grounded, never invents.
async function summarizeStructured(day: string, raw: string): Promise<DailyData> {
   if (raw.startsWith('No task activity')) return { sections: [], pending: [] };
   ...
   if (!key) return { sections: [], pending: [] };
   try {
...
export async function generateDailyUpdate(day: string): Promise<DailyUpdate> {
   ...
   return {
      ...
      data,
      edited: false,
      generated_at: rows[0]?.generated_at ?? new Date().toISOString(),
   };
}
...
export async function listDailyDates(limit = 30): Promise<string[]> {
   ...
   const rows = await query<{ progress: string }>(
      `SELECT coalesce(progress,'') AS progress FROM ops_issues`
   );
